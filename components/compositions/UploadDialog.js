...
export default function UploadingDialog({file, setIsUploading}) {
  ...
  return (
    <div
      className="uploading-dialog fixed dead-center-bottom flex h-24 w-1/2 flex-col items-center rounded-xl bg-violet-600 pb-2 shadow-lg"
...
async function upload(file, setProgress, setIsUploading) {
  ...
  ajax.upload.addEventListener('progress', (event) => progressHandler(event, setProgress), false)
  ajax.addEventListener('load', (event) => completeHandler(setIsUploading), false)
